...
void mlir::nova::createNovaPipelines(OpPassManager &pm) {
  ...
  bufferization::OneShotBufferizePassOptions
      bufferizeOptions; // create a options variable
  bufferizeOptions.bufferizeFunctionBoundaries =
      true; // mention that we want to bufferize function boundaries ie
            // parameters and returns
  bufferizeOptions
      .functionBoundaryTypeConversion = // keep the layout map as identity for
                                        // function boundaries
      bufferization::LayoutMapOption::
          IdentityLayoutMap; // this will ensure that the function signatures
                             // remain unchanged

  bufferizeOptions.useEncodingForMemorySpace = true;

  // so when we bufferize a function that takes a tensor as input, it will be
  // converted to a memref with an identity layout map
  pm.addPass(mlir::bufferization::createOneShotBufferizePass(bufferizeOptions));

  //  Buffer deallocation pipeline - this will make sure that all allocated
  //  buffers are deallocated properly based on the analysis done during
  //  bufferization
...
void mlir::nova::registerNovaPipelines() {
  PassPipelineRegistration<>("nova-opt-pipeline", "Nova Optimizer Pipeline",
                             createNovaPipelines);
}